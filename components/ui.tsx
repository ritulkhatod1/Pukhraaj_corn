...
export function Process({ inverse = false }: { inverse?: boolean }) {
  const steps = [["01", "Sourcing", "Corn is sourced through established agricultural and supplier relationships, with attention to the quality and suitability of incoming raw material.", Sprout], ["02", "Processing", "Selected corn moves through appropriate processing and grading stages to produce consistent corn products for commercial requirements.", Factory], ["03", "Packaging", "Finished products are packed appropriately to help maintain product quality during storage, handling, and transportation.", Package], ["04", "Delivery", "Orders are prepared and dispatched to customers with a focus on dependable supply across India.", Truck]] as const;
...
export function Timeline() {
  ...
  return <div className="mt-10">
    ...
    <article aria-live="polite" className="mt-8 max-w-3xl bg-white p-7 shadow-sm transition duration-500 sm:p-9 md:mt-10"><p className="text-sm font-extrabold tracking-[.18em] text-[#b1830d]">{m.year}</p><h3 className="display mt-3 text-4xl text-[#18382b]">{m.title}</h3><p className="mt-4 text-sm leading-7 text-stone-600">{m.copy}</p><div className="mt-7 inline-flex whitespace-pre-line border-l-4 border-[#d5a51d] bg-[#f4efdf] px-5 py-4 text-sm font-extrabold tracking-wider text-[#18382b]">{m.stat}</div></article>
  </div>;
}
...
export function QuoteForm() {
  ...
  return <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
    {fields.map(([label, name, type]) => <label key={name} className="text-xs font-bold uppercase tracking-wider text-[#435348]">{label}<input required name={name} autoComplete={name === "phone" ? "tel" : name} type={type} className="mt-2 w-full border border-[#d9d1be] bg-[#fffdf8] px-3 py-3 text-sm font-normal outline-none focus:border-[#d5a51d]" /></label>)}
    ...
    <label className="text-xs font-bold uppercase tracking-wider text-[#435348] sm:col-span-2">Message<textarea name="message" className="mt-2 min-h-28 w-full border border-[#d9d1be] bg-[#fffdf8] px-3 py-3 text-sm font-normal outline-none focus:border-[#d5a51d]" /></label>
